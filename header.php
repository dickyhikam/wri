...
$name_menu = [
  ...
  'report_transaksi' => 'Report Transaksi',
  'report_training' => 'Report Training',
  'report_bmp' => 'Report BMP',
  'report_k3' => 'Report K3',
  'report_hcv' => 'Report HCV',
  'petani' => 'Petani',
  'lahan' => 'Lahan/Persil',
  'parcel' => 'Parcel Data',
  'ics' => 'ICS & Fasilitator',
  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-[#403c3c] text-white flex flex-col shadow-xl z-10">
      ...
      <nav class="flex-1 overflow-y-auto py-4 space-y-1 text-sm">
        ...
        <!-- User Management -->
        <div <?php echo ($user['akun']['role'] == 'Super Admin') ? '' : 'hidden'; ?> class="menu-collapse" :class="{'collapsed': !menuCollapse.geospatial}">
          <div @click="menuCollapse.geospatial = !menuCollapse.geospatial" class="sidebar-item flex items-center justify-between px-6 py-3 cursor-pointer">
            <div class="flex items-center">
              <i class="fas fa-map w-5 mr-3 text-[#f0ab00]"></i>
              Geospatial
            </div>
            <i class="fas fa-chevron-down text-xs transition-transform duration-300" :class="{'transform rotate-180': menuCollapse.geospatial}"></i>
          </div>
          <div class="submenu pl-14 pr-6 py-2 space-y-1">
            <a href="geoserver" @click="currentMenu = 'geoserver'" class="block px-4 py-2 rounded-md hover:bg-yellow-300 hover:text-black <?php echo ($name_menu[$modified_uri] == 'Geoserver') ? 'sidebar-item active' : ''; ?>">Geoserver</a>
            <a href="external" @click="currentMenu = 'external'" class="block px-4 py-2 rounded-md hover:bg-yellow-300 hover:text-black <?php echo ($name_menu[$modified_uri] == 'Data External') ? 'sidebar-item active' : ''; ?>">Data External</a>
          </div>
        </div>
