...
<section class="flex-1 overflow-y-auto p-8 bg-gray-50">
    <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 mb-6">
        <h3 class="text-lg font-semibold mb-4">Filter Laporan K3</h3>
        <form id="filterK3Form" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-6">
            <div>
            ...
            <div>
                <label class="text-sm font-medium text-gray-600 mb-1 block">Tipe K3</label>
                <select id="trainingTypeFilter" class="border rounded-lg px-4 py-2 w-full">
                    <option value="">Semua K3</option>
                </select>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border border-gray-100 mb-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="font-semibold text-lg">Daftar Rekap K3</h3>
            <div class="space-x-2">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="rekapK3TableBody">
            </table>
        </div>
    </div>
...
<script>
    ...
    colCheckboxes.forEach(cb => {
        cb.addEventListener('change', toggleK3TableColumns);
    });

    // Panggil sekali saat halaman load untuk set kolom default
    window.addEventListener('DOMContentLoaded', toggleK3TableColumns);

    // Render tabel with class per td
    function renderK3Table(filtered) {
        const tbody = document.getElementById('rekapK3TableBody');
        tbody.innerHTML = `<thead>
                    <tr class="bg-gray-50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase type">Tipe K3</th>
        ...
        toggleK3TableColumns();
    }

    // Render semua data awal
    renderK3Table(trainingData);

    const filterK3Form = document.getElementById('filterK3Form');
    filterK3Form.onsubmit = function(e) {
        e.preventDefault();
        const btnText = document.getElementById('btnFilterText');
        const spinner = document.getElementById('loadingFilterSpinner');
        ...
        // Ambil nilai filter ICS dan K3 Type
        const ics = icsSelect.value;
        const type = typeSelect.value;

        // Filter data training sesuai ICS dan K3 Type
